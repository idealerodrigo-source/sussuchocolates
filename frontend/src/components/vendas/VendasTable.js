...
function ModalReceberPagamento({ venda, onConfirmar, onFechar }) {
  ...
  const handleConfirmar = async () => {
    ...
    if (valor > valorPendente + 0.01) {
      alert(`O valor não pode ser maior que ${formatCurrency(valorPendente)}`);
      return;
    }
    setConfirmando(true);
      ...
      <div className="relative bg-[#FFFDF8] rounded-2xl shadow-2xl p-6 w-full max-w-md mx-4 z-10">
        ...
        <div className="mb-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              type="button"
              onClick={() => setValorRecebido(valorPendente.toFixed(2))}
              className="px-3 py-2 text-xs bg-[#6B4423] text-white rounded-lg hover:bg-[#8B5A3C]"
            >
              Total
            </button>
          </div>
          ...
          {valor > 0 && valor < valorPendente - 0.01 && (
            <div className="mt-2 p-2 bg-orange-50 border border-orange-200 rounded-lg">
              <p className="text-xs text-orange-700">
                ⚠️ Saldo restante após este pagamento: <strong>{formatCurrency(saldoRestante)}</strong> — ficará como "A Receber"
              </p>
            </div>
          )}
          {quitaTotal && valor > 0 && (
        ...
        <div className="flex gap-3">
          ...
          <Button
            ...
            disabled={!formaPagamento || confirmando || valor <= 0}
...
function ModalEditarPagamento({ venda, onConfirmar, onFechar }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-[#FFFDF8] rounded-2xl shadow-2xl p-6 w-full max-w-md mx-4 z-10">
        <div className="flex items-center justify-between mb-5">
          <h2 className="text-xl font-serif font-bold text-[#3E2723]">Editar Forma de Pagamento</h2>
          <button onClick={onFechar} className="text-[#8B5A3C] hover:text-[#6B4423] p-1">
            <X size={20} weight="bold" />
          </button>
        </div>
        <div className="bg-[#F5E6D3] rounded-xl p-4 mb-5">
          ...
          <p className="font-semibold text-[#3E2723]">{venda.cliente_nome}</p>
          <div className="flex justify-between mt-2">
            <p className="text-sm text-[#705A4D]">Valor da venda</p>
            <p className="text-lg font-bold text-[#3E2723]">{formatCurrency(venda.valor_total)}</p>
          </div>
          <div className="flex justify-between mt-1">
            <p className="text-sm text-[#705A4D]">Pagamento atual</p>
            <p className="text-sm font-medium text-[#8B5A3C]">{venda.forma_pagamento}</p>
          </div>
        </div>
        <div className="mb-5">
          <p className="text-sm font-semibold text-[#6B4423] mb-3">Selecione a forma correta:</p>
          <div className="grid grid-cols-2 gap-2">
            {FORMAS_PAGAMENTO.map((forma) => (
              <button
                key={forma}
        ...
        <div className="flex gap-3">
          ...
          <Button
            ...
            disabled={!formaPagamento || salvando || formaPagamento === venda.forma_pagamento}
...
export default function VendasTable({
  ...
  const handleEditarPagamento = async (vendaId, formaPagamento) => {
    await onEditarPagamento(vendaId, formaPagamento);
    ...
    <>
      ...
      {vendaParaEditar && (
        <ModalEditarPagamento
          ...
          onConfirmar={handleEditarPagamento}
          onFechar={() => setVendaParaEditar(null)}
        />
      )}
                  ...
                  <tr key={venda.id} className={`border-t border-[#8B5A3C]/10 hover:bg-[#F5E6D3]/50 ${venda.status_venda === 'cancelada' ? 'bg-red-50/50 opacity-75' : ''}`}>
                    <td className="px-6 py-4">
                      <div className="flex flex-col gap-1">
                        <span className={`px-2 py-1 rounded text-xs font-medium inline-block w-fit ${
                          venda.tipo_venda === 'direta' ? 'bg-purple-100 text-purple-700' : 'bg-blue-100 text-blue-700'
                        }`}>
                      ...
                      <div className="flex flex-col gap-0.5">
                        ...
                        {venda.valor_pendente && venda.valor_pendente < venda.valor_total && (
                          <span className="text-[10px] text-orange-600 font-medium">
                            Pago: {venda.valor_pago_parcial ? `R$ ${venda.valor_pago_parcial.toFixed(2).replace('.', ',')}` : ''}
                          </span>
                        )}
                      </div>
                    </td>
                    <td className="px-6 py-4 text-sm text-[#4A3B32] font-sans text-right font-medium">
                      <div className="flex flex-col items-end gap-0.5">
                        ...
                        {venda.valor_pendente && venda.valor_pendente < venda.valor_total && (
                          <span className="text-[10px] text-orange-600 font-medium">
                            Saldo: {formatCurrency(venda.valor_pendente)}
                          </span>
                        )}
                      </div>
                    </td>
                      ...
                      <div className="flex gap-2 justify-end">
                        {venda.status_venda !== 'cancelada' && (
                          <>
                            ...
                            {venda.status_pagamento === 'pago' && !venda.nfce_emitida && (
                              <Button
                              ...
                              >
                                <PencilSimple size={16} weight="bold" className="mr-1" />
                                Pgto
                              </Button>
                            )}
                            {!venda.nfce_emitida && (
